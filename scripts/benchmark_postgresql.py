    ...
    for _ in range(iterations):
        with SessionLocal() as session:
            ...
            predictions = session.execute(
                ...
                .where(Prediction.created_at > datetime.utcnow() - timedelta(days=7))
    ...
    def worker_task():
        ...
        for _ in range(queries_per_worker):
            with SessionLocal() as session:
                ...
                predictions = session.execute(
                    select(Prediction)
                    .where(Prediction.created_at > datetime.utcnow() - timedelta(days=1))
                    .limit(10)
                ).all()
    ...
    for _ in range(iterations):
        with SessionLocal() as session:
            ...
            results = session.execute(
                ...
                .where(Prediction.created_at > datetime.utcnow() - timedelta(days=30))
                .order_by(Prediction.created_at.desc())
                .limit(100)
            ).all()
