...
def get_statistics_metrics(engine, date_range=None, granularity="all"):
    ...
    try:
        with Session(engine) as db:
            ...
            simulations_query = db.query(func.count(TradingSimulation.simulation_id))
            if start_date:
                simulations_query = simulations_query.filter(TradingSimulation.created_at >= start_date)
            if end_date:
                simulations_query = simulations_query.filter(TradingSimulation.created_at <= end_date)
            total_simulations = simulations_query.scalar() or 0
        ...
        return html.Div([
            dbc.Row([
                ...
                dbc.Col(metric_card("🧠", "LLM", f"{total_processed:,}", "text-success",
